...
def data_info(data_copy, save_dir):
    ...
    print(f"Data info saved in: {info_file_path}")
...
def dataset_info_shap_datatypes_num_cat_count_duplecatedrows(df, save_dir):
    ...
    output_text.append(info_table.to_markdown(index=False, tablefmt="github"))

    data_types_count = df.dtypes.value_counts().reset_index()
    data_types_count.columns = ["Data Type", "Count of Columns"]
    output_text.append("\nData types with counts of columns:\n")
    output_text.append(data_types_count.to_markdown(index=False, tablefmt="github"))

    # Add a table for numerical and categorical variable counts
    var_counts_data = {
    ...
    output_text.append(var_counts_table.to_markdown(index=False, tablefmt="github"))

    if not duplicated_rows.empty:
        output_text.append("\nDetailed list of duplicated rows (including indices):\n")
        output_text.append(duplicated_rows.to_markdown(index=True, tablefmt="github"))
    else:
    ...
    print(f"\nDataset information saved to: {output_file_path}")
...
def save_and_remove_duplicates(df, save_dir):
    ...
    if not duplicated_rows.empty:
        ...
        print(f"Duplicated rows saved to: {duplicates_file}")

        # Remove duplicated rows from the DataFrame
        df_cleaned = df.drop_duplicates(keep='first')
        print("Duplicated rows removed from the DataFrame.")
    else:
...
def analyze_missing_values(data_copy, save_dir):
    ...
    user_input = input("Do you want to convert any values to NaN? (y/n): ").strip().lower()
    if user_input == 'y':
        values_to_replace = input("Enter values to convert to NaN (comma-separated): ").strip().split(',')
    ...
    print(f"Missing values report saved at: {report_file_path}")
    ...
    plt.close()
    print(f"Missing values matrix plot saved at: {matrix_plot_path}")
    ...
    plt.close()
    print(f"Missing values bar chart saved at: {bar_plot_path}")
...
def detect_outliers(data, dtypes, save_dir):
    ...
    print(f"\nOutliers summary saved to: {output_file_path}")
...
def detect_iqr_outliers_all(data_copy, save_dir, output_filename="iqr_outliers_report.csv"):
    ...
    print(f"Outlier detection report saved successfully at: {output_path}")
...
def numerical_features_boxplot_skew_kurto_outliers(data_copy, save_dir):
    ...
    print(f"Structured statistical summary saved at: {report_path}")
    ...
    print("All individual box plots have been saved successfully in the 'outliers' folder.")
...
def data_table_range_min_max_distinct(data_copy, save_dir):
    ...
    print(f"Column summary saved to: {result_path}")
...
def TableOne_groupby_column(data_copy, save_dir):
    ...
    print("1: Single column report")
    print("2: Multiple columns report")
    print("3: All columns report")
    report_type = input("Enter your choice (1/2/3): ").strip()
    
    groupby_columns = []
    
    if report_type == "1":
        column_name = input(f"Enter a column name for grouping (available: {all_columns}): ").strip()
        if column_name in data_copy.columns:
            groupby_columns = [column_name]
        else:
            print(f"Column '{column_name}' not found. Exiting.")
            return
    
    elif report_type == "2":
        print(f"Available columns: {all_columns}")
        columns = input("Enter column names for grouping, separated by commas: ").strip().split(",")
    ...
    for groupby_column in groupby_columns:
        print(f"Generating report for grouping by: {groupby_column}")
        ...
        print(f"Summary table for grouping by '{groupby_column}' saved as: {csv_output_path}")
...
def analyze_relation(data_copy, save_dir):
    ...
    def process_target():
        target_available = input("Is there a target variable? (y/n): ").lower()
        if target_available == 'y':
            target_var = input("Enter the target variable name: ").strip()
            if target_var in data_copy.columns:
                return target_var
            else:
                print(f" Target variable '{target_var}' not found in the dataset. Skipping target-based analysis.")
                return None
        return None
    ...
    if target_series.dtype == 'object' or target_series.dtype.name == 'category':
        ...
        if len(unique_values) == 2:
            ...
            print(f" Converted categorical target '{target_var}' using mapping: {mapping}")
        else:
            print(f" Warning: Target variable '{target_var}' has more than two categories. Skipping correlation analysis.")
            generate_clustered_correlation_heatmap()
            return

    #  Step 6: Chi-Square Test for Categorical Variables
    print(" Performing chi-square test for categorical variables...")
    ...
    print(" Performing correlation analysis for numerical variables...")
    ...
    print(f" Results saved in CSV and TXT format at {results_dir}")
...
def plot_summary_num_hist_box_optional_target(data, save_dir):
    ...
    target_input = input("Do you want to analyze variables with a target variable? (y/n): ").strip().lower()
    target = None
    if target_input == 'y':
    ...
    print(f"\nAll outputs (analyses and plots) have been saved in: {vis_dir}")
...
def plot_num_kde_subplot(data_copy, save_dir, layout_title="KDE Plots of Numerical Variables"):
    ...
    print("All plots have been saved successfully in the 'plot_num_kde_subplot' folder.")
...
def plot_num_box_plots_all(data_copy, save_dir, layout_title="Box Plots of Numerical Variables"):
    ...
    print("All box plots have been saved successfully in the 'plot_num_box_plots_all' folder.")
...
def plot_num_qq_subplot(data_copy, save_dir, layout_title="QQ Plots of Numerical Variables"):
    ...
    print("All QQ plots have been saved successfully in the 'plot_num_qq_subplot' folder.")
...
def plot_summary_cat_bar_optional_target(data, save_dir):
    ...
    print(f"\nAll categorical variable analyses have been saved in: {vis_dir}")
...
def plot_cat_pie_charts_subplot(data_copy, save_dir, layout_title="Pie Charts of Categorical Variables"):
    ...
    print("All pie charts have been saved successfully in the 'plot_cat_pie_charts_subplot' folder.")
...
def plot_pair_scatter_numerical_vs_numerical(data_copy, save_dir):
    ...
    use_hue = input("Do you have a target variable for coloring the scatter plots? (y/n): ").strip().lower()
    hue_column = None

    if use_hue == 'y':
        hue_column = input("Enter the target variable name (column): ").strip()
    ...
    print("All numerical vs numerical plots have been saved successfully.")
...
def plot_pair_heatmaps_categorical_vs_categorical(data_copy, save_dir):
    ...
    print("All categorical vs categorical plots have been saved successfully.")
...
def main(file_path, save_dir):
    ...
    try:
        ...
        if not delimiter:
            print("Delimiter not detected automatically.")
            print("Displaying top 5 rows of the data to help determine the correct delimiter.")
            temp_df = pd.read_csv(file_path, nrows=5)
            print(temp_df.head())
            delimiter = input("Please enter the delimiter manually (e.g., ',', ';', '\t'): ").strip()
        ...
        run_save_remove_duplicates = input("Do you want to remove duplicates? (y/n): ").strip().lower()
        ...
        run_missing_values_analysis = input("Do you want to analyze missing values? (y/n): ").strip().lower()
        if run_missing_values_analysis == 'y':
            analyze_missing_values(data_copy, save_dir)

        run_auto_impute = input("Do you want to auto-impute missing values? (y/n): ").strip().lower()
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python auto_data_analysis.py <path_to_csv_file> <output_directory>")
    else:
