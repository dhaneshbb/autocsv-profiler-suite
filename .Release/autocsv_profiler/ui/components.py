...
Rich-styled versions of the traditional console functions with styled formatting.
"""
...
def _import_log_print() -> Optional[Callable]:
...
def _import_types() -> tuple[type | None, type | None]:
    """Lazy import types only when needed."""
    try:
        from autocsv_profiler.types import ColumnName, PathLike

        return ColumnName, PathLike  # type: ignore
    except ImportError:
        return None, None


class RichInteractiveComponents:
    """Rich-styled interactive components for CSV analysis"""
    ...
    def exclude_columns_rich(
        self, data_copy: "pd.DataFrame", save_dir: str, delimiter: str = ","
    ) -> "pd.DataFrame":
        """
        Rich-styled column exclusion interface
        ...
        pd = _import_pandas()
        numpy = _import_numpy()
        if pd is None or numpy is None:
        ...
        column_table.add_column("CATEGORICAL COLUMNS", style="cyan", width=35)
        column_table.add_column("NUMERICAL COLUMNS", style="green", width=35)
    ...
    def tableone_column_selection_rich(
        self, data_copy: "pd.DataFrame"
    ) -> Dict[int, str]:
        """
        Rich-styled TableOne column selection
        ...
        for i in range(max_items):
            ...
            if categorical_with_idx and i < len(categorical_with_idx):
                ...
                row_data.append(f"[{idx:2}] {col_name}")
            elif len(tableone_table.columns) > 1:
                row_data.append("")
            ...
            if len(row_data) == 1 and len(tableone_table.columns) > 1:
                row_data.append("")
    ...
    def visualization_selection_rich(self) -> List[Dict[str, Any]]:
        ...
        for i, viz in enumerate(visualizations):
            viz["index"] = i
            viz_table.add_row(str(i), str(viz["name"]), str(viz["description"]))
        ...
        if selection == "" or selection in ["skip", "none"]:
            ...
            try:
                ...
                selected_names = [str(viz["name"]) for viz in selected_visualizations]
                self.console.print(
...
def exclude_columns_styled(
    ...
    """
    Rich-styled version of exclude_columns function with styled UI
...
def display_columns_indexed_styled(
    data_copy: "pd.DataFrame", console: Optional[Console] = None
) -> Dict[int, str]:
    """
    Rich-styled version of display_columns_indexed function with styled UI
...
def display_visualizations_indexed_styled(
    console: Optional[Console] = None,
) -> List[Dict[str, Any]]:
    """
    Rich-styled version of display_visualizations_indexed function with styled UI
...
def TableOne_groupby_column_styled(
    ...
    """
    Rich-styled version of TableOne_groupby_column with styled UI
...
def TableOne_groupby_column_original(
    ...
    """
    Original TableOne implementation - called by styled version
    ...
    try:
        ...
        print("WARNING: tableone library not available. Skipping TableOne analysis.")
        return
...
def select_and_execute_visualizations_styled(
    ...
    """
    Rich-styled version of select_and_execute_visualizations with styled UI
