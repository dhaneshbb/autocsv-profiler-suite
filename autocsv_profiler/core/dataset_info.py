...
def data_table_range_min_max_distinct(
    data_copy: pd.DataFrame, save_dir: str, delimiter: str = ","
) -> None:
    """
...
def generate_complete_report(
    df: pd.DataFrame, save_dir: str, file_path: str, delimiter: str = ","
) -> str:
    ...
    os.makedirs(save_dir, exist_ok=True)
    combined_analysis_path = os.path.join(save_dir, "dataset_analysis.txt")
    with open(combined_analysis_path, "w", encoding="utf-8") as f:
        f.write("\n".join(complete_output))

    if not num_summary_df.empty:
        num_summary_path = os.path.join(save_dir, "numerical_summary.csv")
        num_summary_df.to_csv(num_summary_path, sep=delimiter)

    if not cat_summary_df.empty:
        cat_summary_path = os.path.join(save_dir, "categorical_summary.csv")
...
def distinct_val_tabular_txt(
    data: "pd.DataFrame",
    output_file: str = "distinct_values.txt",
    ...
    def capture_columns_info(data: "pd.DataFrame") -> str:
    ...
    def create_side_by_side_table(
        table_data: list, headers: list, max_rows_per_column: int, max_columns: int
    ) -> str:
        ...
        if total_rows <= max_rows_per_column:
            return str(
                tabulate(table_data, headers=headers, tablefmt="pipe", stralign="left")
            )
